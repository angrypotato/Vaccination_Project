...
coord_info <-  coord_info[-c(402),]

for(i in NROW(pj)){
  if(pj$TEHSIL[i] %not in% facilities$TEHSIL){
    print(pj$TEHSIL[i])
  }
}
inc <- 0
oc <- 0
for(facility in 1:NROW(facilities)){
  fac <- facilities[facility,]
  sclinic_f <- f[(f$long >= fac$longitude_low & f$long <= fac$longitude_high
                  & f$lat <= fac$latitude_high & f$lat >= fac$latitude_low),]
  #num_total <- NROW(f)
  inc <- inc + NROW(sclinic_f)
  
}
oc <- NROW(f) - inc

pj<-coord_info[(coord_info$p_included==1),]
head(pj)
for(i in NROW(pj)){
  if(pj$TEHSIL[i] %in% facilities$TEHSIL){
    #do nothing
  }
  else{
    print(pj$TEHSIL[i])
...
m1 <- clean_df2(files_17[1])
m2 <- clean_df2(files_17[2])
morethan <- rbind(m1,m2)
NROW(morethan)

f <- clean_df2(files_17[1])

pj$penta1_ratio <-pj$penta1_in_clinic/(pj$penta1_out_clinic + pj$penta1_in_clinic)
pj$penta3_ratio <-pj$penta3_in_clinic/(pj$penta3_out_clinic + pj$penta3_in_clinic)
pj$measles_ratio <-pj$measles_in_clinic/(pj$measles_out_clinic + pj$measles_in_clinic)

coord_info[(coord_info$penta1_out_clinic < 0),]$penta1_out_clinic <- 0
pj[(pj$penta3_out_clinic < 0),]$penta3_out_clinic <- 0
pj[(pj$penta1_out_clinic < 0),]$penta1_out_clinic <- 0
pj[(pj$measles_out_clinic < 0),]$measles_out_clinic <- 0

for(i in 1:NROW(punjab_covars)){
  print(1)
  tt <- punjab_covars$TEHSIL[i]
  print(tt)
  punjab_covars[(punjab_covars$TEHSIL == tt),]$measles_ratio <- pj[(pj$TEHSIL == tt),]$measles_ratio
  print(1)
  punjab_covars[(punjab_covars$TEHSIL == tt),]$penta1_ratio <- pj[(pj$TEHSIL == tt),]$penta1_ratio
  print(1)
  punjab_covars[(punjab_covars$TEHSIL == tt),]$penta3_ratio <- pj[(pj$TEHSIL == tt),]$penta3_ratio
  print(1)
}

punjab_covars <- punjab_covars[-c(115),]
punjab <- coord_info[(coord_info$p_included==1),]
View(pj[,c("TEHSIL","penta1_ratio","penta3_ratio")])
h_tehsils <- i_tehsils
tehsils <- i_tehsils
head(aggregate(penta3 ~ Type, facilities, sum))

subset_o_tehsils <- j_tehsils[,c(11,12,13,33,35,36,37,38)]
subset_tehsils <- tehsils[,c(10,11,12,13,33,35,36,37,38,41,44,47,50,53,56,59,62,65,68,74)]
