...
coefs <- data.frame("fertility"=NA, "elevation"=NA, "poverty"=NA, "night_lights"=NA,"Population"=NA,
                    "child_population"=NA, "population_density"=NA, "radio"=NA, "electricity"=NA,"mobile_phone"=NA,"antenatal_care"=NA,
                    "mothers_age"=NA)
...
for (i in 1:1000) {
  ...
  ratio.step <- gbm.step(
    data=sample_d, 
    gbm.x = c(1:4, 7:11, 13,15,16),   # selected features 
    gbm.y = 20,
    family = "gaussian",
    tree.complexity = 2,
    learning.rate = 0.005,
...
adj.r2(R2(pentaTrain[,20],ratio_gam_preds),96,7)
...
gam.form <- as.formula(OutreachProportion ~ s(fertility, k=5)+s(elevation, k=5)  + s(poverty, k=5)  + s(night_lights, k=5)+s(Population, k=5)+s(child_population, k=5)+
                         s(population_density,k=5) +s(radio, k=5) + s(electricity, k=5)+s(mobile_phone, k=5)+ + s(antenatal_care, k=5) + s(mothers_age, k=5) )
...
for (i in 1:1000) {
  ...
  names(clinic_gam_cfs) <- c("fertility", "elevation", "poverty", "night_lights","Population",
                             "child_population", "population_density", "radio", "electricity","mobile_phone","antenatal_care",
                             "mothers_age")
